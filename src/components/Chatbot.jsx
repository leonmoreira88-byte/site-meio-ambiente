      ...
      {isOpen && (
        <div className="bg-white w-[90vw] sm:w-96 h-[500px] max-h-[80vh] rounded-2xl shadow-2xl border border-green-100 flex flex-col overflow-hidden">
          <div className="bg-gradient-to-r from-green-800 to-green-700 text-white p-4 flex justify-between items-center">
            <h3 className="font-bold">🤖 Robschat</h3>
            <button onClick={() => setIsOpen(false)}>✕</button>
          </div>

          <div className="flex-1 p-4 overflow-y-auto bg-[#f4f8f4] space-y-3">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white border border-green-200 text-gray-500 rounded-xl rounded-bl-none p-3 text-sm shadow-sm">
                  Pensando...
                </div>
              </div>
            )}
          ...
          <form
            ...
            className="p-3 border-t border-green-100 bg-white flex gap-2"
          >
            <input
              type="text"
              value={input}
              ...
              className="flex-1 rounded-xl border border-green-200 px-4 py-2 text-sm outline-none"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="bg-green-700 text-white px-4 py-2 rounded-xl disabled:opacity-50"
            >
              Enviar
            </button>
          </form>
